...
dataPlot <- states %>%
  inner_join(data) #join data with maps information
...
p1 <- dataPlot %>%
  ...
  geom_polygon( aes(x = long, y = lat,
                    group = group, fill = percentage),color = "black" ) +
  scale_fill_gradient2(name = "%",low = "white",
                       mid = "#A770CD", high = "#410066", midpoint = 55) +
...
p2 <- dataPlot %>%
  ...
  ggplot() + 
  geom_polygon( aes(x = long, y = lat, 
                    group = group, fill = invers), color = "black" ) +
  scale_fill_gradient2(name = "%", low = "white",
                       mid = "chartreuse3", high = "antiquewhite4", midpoint = 45) +
  labs(title = "Non-white people in the US colleges", 
       subtitle = "Southern states have more \n Non-white people than Northern") +
  theme_ari() +
  theme(plot.title = element_text(family = font,face = "bold",
...
p3 <- data2 %>%
  ...
  scale_color_manual(values=c( "chartreuse3", "#410066"))+
  theme_minimal()+
  coord_flip()+
  facet_wrap(~state, ncol = 5)+
  ylab("")+
  xlab("")+
  labs(caption= "    by @AnguloBrunet \n    Data from Tuitiontracker.org \n    #tidytuesday") +
  theme(legend.position ="none", 
        strip.background = element_rect(color = colorfondo, fill = colorfondo), 
