...
headsh <-  season_goals %>%
  ...
  group_by(player) %>%
  filter(season %in% (season)) %>% #choose only first season
...
plot1 <-  ggplot()+
  geom_point(data=dat,aes(y=reorder(player, desc(rank)),x = season, 
                          size = total, colour = status),alpha =.5) +
  geom_image(data=headsh,aes( y = player, x = season,
                              image = headshot), size = 0.05) + #adds image
  ...
  shadow_mark()
animate(plot1, nframes = 2 * length(unique(dat$season)), 
        height = 1748/2, width = 2480/2) 
